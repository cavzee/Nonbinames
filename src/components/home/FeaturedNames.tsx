      ...
      <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-5">
        {names.map((name) => (
          <Link
            key={name!.slug}
            href={`/name/${name!.slug}`}
            className="group rounded-3xl border border-zinc-800 bg-zinc-900/40 p-6 transition duration-300 hover:-translate-y-1 hover:border-violet-500/40 hover:bg-zinc-900"
          >
            ...
            <div className="mt-8 text-sm font-medium text-violet-400 transition group-hover:translate-x-1">
              Discover →
            </div>
          </Link>
        ))}
