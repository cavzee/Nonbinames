...
export function NameCard({ name }: Props) {
  return (
    <Link href={`/name/${name.slug}`} className="group block">
      <article
      ...
      >
        ...
        <div className="mt-8 text-sm font-medium text-violet-400 opacity-0 transition-all duration-300 group-hover:translate-x-1 group-hover:opacity-100">
          Discover →
        </div>
      </article>
    </Link>
